...
const edgeServerSrcPromise = fs_1.promises.readFile((0, path_1.join)(__dirname, '../server-edge.mjs'), 'utf-8');
const nodeServerSrcPromise = fs_1.promises.readFile((0, path_1.join)(__dirname, '../server-node.mjs'), 'utf-8');
const build = async ({ entrypoint, files, workPath, repoRootPath, config, meta = {}, }) => {
    ...
    const nodeModulesDir = (0, path_1.join)(entrypointFsDirname, 'node_modules');
    const remixRunDevPath = (0, path_1.join)(nodeModulesDir, '@remix-run/dev');
    let backupRemixRunDevPath = `${remixRunDevPath}.__vercel_backup`;
    try {
    ...
    const serverBundles = Array.from(serverBundlesMap.entries()).map(([hash, routes]) => {
    ...
    const [staticFiles, ...functions] = await Promise.all([
        ...
        ...serverBundles.map(bundle => {
            ...
            if (config.runtime === 'edge') {
                return createRenderEdgeFunction(entrypointFsDirname, repoRootPath, (0, path_1.join)(entrypointFsDirname, bundle.serverBuildPath), remixConfig.serverEntryPoint, remixVersion, config);
            }
            return createRenderNodeFunction(nodeVersion, entrypointFsDirname, repoRootPath, (0, path_1.join)(entrypointFsDirname, bundle.serverBuildPath), remixConfig.serverEntryPoint, remixVersion, config);
        }),
    ]);
